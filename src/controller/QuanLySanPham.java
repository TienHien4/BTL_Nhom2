...
public class QuanLySanPham {
	...
	public void load_SanPham() {
		...
		for (int i = 0; i < ds_Sach.size(); i++) {
			model.addRow(new Object[] { i + 1, ds_Sach.get(i).getMaSanPham(), ds_Sach.get(i).getTenSanPham(), "Sach",
					ds_Sach.get(i).getGiaMua(), ds_Sach.get(i).getGiaBan(), ds_Sach.get(i).getSoLuong() });
		}
		for (int i = 0; i < ds_DiaNhac.size(); i++) {
			model.addRow(new Object[] { i + 1 + ds_Sach.size(), ds_DiaNhac.get(i).getMaSanPham(),
					ds_DiaNhac.get(i).getTenSanPham(), "Đĩa Nhạc", ds_DiaNhac.get(i).getGiaMua(),
					ds_DiaNhac.get(i).getGiaBan(), ds_DiaNhac.get(i).getSoLuong() });
		}
		for (int i = 0; i < ds_DiaPhim.size(); i++) {
			model.addRow(new Object[] { i + 1 + ds_Sach.size() + ds_DiaNhac.size(), ds_DiaPhim.get(i).getMaSanPham(),
					ds_DiaPhim.get(i).getTenSanPham(), "Đĩa Phim", ds_DiaPhim.get(i).getGiaMua(),
					ds_DiaPhim.get(i).getGiaBan(), ds_DiaPhim.get(i).getSoLuong() });
		}
	}
	...
	public void themSanPham(SanPham sp) {
		...
		load_SanPham();
		if (QuanLyDangNhap.isQuanLi)
			FrameQuanLy.showThongKeSP(thongKe());
		if (QuanLyDangNhap.isNhanVien)
			FrameNhanVien.showThongKeSP(thongKe());
	}
	...
	public void suaThongTin(int i) {
		...
		if (i < sizeSach) {
			...
			fr.setVisible(true);
		} else if (i < sizeSach + sizeDiaNhac) {
